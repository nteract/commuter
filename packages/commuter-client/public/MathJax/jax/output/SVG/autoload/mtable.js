...
MathJax.Hub.Register.StartupHook("SVG Jax Ready", function() {
  ...
  a.mtable.Augment({
    toSVG: function(aa) {
      this.SVGgetStyles();
      var o = this.SVG(), ap = this.SVGgetScale(o);
      ...
      if (aN.equalrows) {
        var U = Math.max.apply(Math, z), aq = Math.max.apply(Math, I);
        for ((aI = 0), (aE = O.length); aI < aE; aI++) {
          az = (U + aq - (z[aI] + I[aI])) / 2;
          z[aI] += az;
          I[aI] += az;
      ...
      var at, ax = 0, V = 0, X = 0, aC = 0, aJ = 0, r = [], B = [], aj = 1;
